...
closure object) is a non-union class type with a unique name,
...
\end{itemize}
For each entity captured by copy, a
non-static data member with a unique name is declared in the closure type. The declaration order of
...
entity captured by copy is transformed into an access to the corresponding data
...
transformed to instead refer to the corresponding data member of the closure type.
\begin{example}
\begin{codeblock}
void f(const int*);
void g() {
...
non-static data members with unique names are declared in the closure type for entities captured by
reference.
